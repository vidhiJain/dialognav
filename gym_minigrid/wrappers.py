...
class HumanFOVWrapper(gym.core.Wrapper):
    ...
    """
    def __init__(self, env, turn_angle=15., yaw=-90.0, agent_pos=None, angle=51, visible_granularity=1):
        super().__init__(env)
        ...
        observation_space = gym.spaces.Box(
            low=0,
            high=255,
            shape=(env.width*visible_granularity, env.height*visible_granularity, 3),
            dtype='uint8'
        )
    ...
    def inview2D_with_opaque_objects(self, yaw, distance_resolution=2, angle_resolution=1):
        ...
        angle_resolution = self.angle*angle_resolution
        dist_resolution = self.distance*distance_resolution
        visible_grid = np.zeros((self.grid.height, self.grid.width), dtype=np.uint8) 

        thetas_in_deg_array = np.linspace(yaw-self.angle, yaw+self.angle, angle_resolution) 
        radius_array = np.linspace(0.01, self.distance, dist_resolution) 

        radius, theta = np.meshgrid(radius_array, thetas_in_deg_array, sparse=True)

        visible_grid[int(round(self.agent_pos[1]))][int(round(self.agent_pos[0]))] = 1
        coord_z = radius*np.cos(theta * np.pi/180) + self.agent_pos[1] # zpos - self.origin_coord['z']
        coord_x = -radius*np.sin(theta * np.pi/180) + self.agent_pos[0] # xpos - self.origin_coord['x']
    ...
    def gen_obs(self):
        obs = self.inview2D_with_opaque_objects(self.yaw)
        self.visible_grid = obs
        return obs
    ...
    def step(self, action):
        ...
        self.env.unwrapped.agent_dir = round((self.yaw - 45.)/45. % 8)
        # print('yaw', self.yaw)
    ...
    def render(self, mode='rgb_array', highlight=True, tile_size=8, 
        ...
        if highlight:
            ...
            def preprocess(array, visible_granularity, tile_size):
                """scale, n, stack"""
            ...
            observed_img = np.where(
                preprocess(self.visible_grid, self.visible_granularity, tile_size), 
                (mgimg + alpha_visiblity*(255-mgimg)).clip(0, 255).astype(np.uint8),
                np.where(preprocess(self.observed_absolute_map, self.visible_granularity, tile_size), 
                    mgimg, 

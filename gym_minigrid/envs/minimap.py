...
class MinimapForSparky(NumpyMap):

    def __init__(self, raw_map_path=Path(RESOURCES_DIR, 'sparky_map.npy'), agent_pos=(4, 25), agent_dir=0):
        raw_map = np.load(raw_map_path)
        minimap_array, modified_index_mapping = get_minimap_from_voxel(
            raw_map, minigrid_index_mapping, jump_index=11)
        super().__init__(
...
class MinimapForFalcon(NumpyMap):

    def __init__(self, raw_map_path=Path(RESOURCES_DIR, 'falcon_map.npy'), agent_pos=(2, 10), agent_dir=0):
        raw_map = np.load(raw_map_path)
        # To ignore the empty grounds in the start

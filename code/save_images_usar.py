...
# def get_big_image(arr):
#     y, z, x = arr.shape

#     # image = np.zeros((y, z*3, x*3))
#     pixel_list = []
#     for z_x_grid in arr:
#         for x_col in z_x_grid:
#             for val in x_col:
#                 # pixel_list.append(map_grid_custom_colors[int(val)])

#     np.array([[[map_grid_custom_colors[int(val)] for val in row_x] for row_x in z_x_grid] for z_x_grid in self.absolute_map], dtype='B')
...
class PlanningAgent():
    ...
    def inview2D_with_opaque_objects(self, grid, yaw, distance, angle=60):
        ...
        for theta_in_deg in range(int(yaw-angle), int(yaw+angle)):
            ...
            for r in range(1, int(distance)):
                ...
                # if theta_in_deg == int(yaw-angle) or theta_in_deg == int(yaw+angle)-1:
                ...
                if isOpaque(grid[z][x]):
                    ...
                    visible_grid[z][x] = 1
                    # if is_passable(visible_grid[z][x]):

        return visible_grid


    def any_neighbours_not_visible(self, y, z, x):
        # self.absolute_map[y, z+, x]
        # passable_neighbours = {}
        for dx in [-1, 1]:
            if x + dx > 0 and x + dx < grid.shape[1]:
                if self.absolute_map[y, z, x+dx] == 0:
                    return True
        for dz in [-1, 1]:
            if z + dz > 0 and z + dz < grid.shape[0]:
                if self.absolute_map[y, z+dz, x] == 0:
                    return True        
        return False


    def aggregate_frontiers(self, y=1):
        for z in self.envsize:
            for x in self.envsize:
                if self.absolute_map[y, z, x] != 0: # is visible
                    if self.is_passable(self.absolute_map[y, z, x]):
                        if self.any_neighbours_not_visible(y,z,x):
                            self.frontier.add((z,x))
                        else:
    ...
    def observe(self):
        ...
        if world_state.number_of_observations_since_last_state > 0:
            ...
            # blended_image = np.array([[[map_grid_custom_colors[val] for val in row_x] for row_x in z_x_grid] for z_x_grid in self.masked_grid], dtype='B')

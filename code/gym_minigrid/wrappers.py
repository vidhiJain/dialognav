...
class Execute_n_Render(gym.core.Wrapper):
    def __init__(self, env):
        super().__init__(env)
...
class View2DFrontiers(gym.core.Wrapper):
    """
    Wrapper to receive only object index 
    and function to retrieve current frontiers 
    based on agent_view_size
    """
    def __init__(self, env, agent_view_size=7):
        super().__init__(env)
    ...
    def get_frontiers(self, obs):
        """
        Gives relative frontier coordinates wrt agent 
        """
        frontiers = {'index':[], 'distance':[]}
        arr = obs['image'][:,:,0].T
        agent_pos = (self.agent_view_size-1,(self.agent_view_size-1)//2)
        for i in range(0, self.agent_view_size):
            if arr[i][0] == 1:
                dist = self.dist_func(a=(i, 0), b=agent_pos)
                # frontiers[dist] += [i, 0]
                frontiers['index'].append((i, 0))
                frontiers['distance'].append(dist)
            
            if arr[i][self.agent_view_size-1] == 1:
                dist = self.dist_func(a=(i, self.agent_view_size-1), b=agent_pos)
                # frontiers[dist] += [i, self.agent_view_size-1]
                frontiers['index'].append((i, self.agent_view_size-1))
                frontiers['distance'].append(dist)
                # frontiers.add([i, self.agent_view_size-1])
            
            if i == 0 or i == self.agent_view_size-1:
                continue
            else:
                if arr[0][i] == 1:
                    dist = self.dist_func(a=(0, i), b=agent_pos)
                    frontiers['index'].append((0, i))
                    frontiers['distance'].append(dist)
                    # frontiers[dist] += [0, i]
                    # frontiers.add([0, i])
                
                if arr[self.agent_view_size-1][i] == 1:
                    dist = self.dist_func(a=(self.agent_view_size-1, i), b=agent_pos)
                    frontiers['index'].append((self.agent_view_size-1, i))
                    frontiers['distance'].append(dist)                
                    # frontiers[dist] += [self.agent_view_size-1, i]
                    # frontiers.add([self.agent_view_size-1, i])
        return frontiers

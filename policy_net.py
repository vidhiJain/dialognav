...
class ActorCritic(nn.Module):
    def __init__(self, input_dim=20, hidden_dim=10):
        ...
        self._policy = nn.Linear(hidden_dim, hidden_dim)
        
    def forward(self, x):
        z = F.relu(self.linear(x))
        
        value = self._value(z)
        dist = self.dist(self._policy(z))
    ...
    def update(self, rollouts):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                values, action_log_probs, dist_entropy = self.actor_critic.evaluate_actions(obs_batch,
...
def main():
    ...
    env = gym_minigrid.wrappers.ViewSizeWrapper(agent_view_size=3)
    obs = env.reset()
    
    breakpoint()
    actor_critic = ActorCritic(input_dim=3*3*3, hidden_dim=10)

    model = PPO(actor_critic, actor_critic)
